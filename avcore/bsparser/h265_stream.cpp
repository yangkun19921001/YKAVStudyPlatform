...
void h265_read_ref_pic_lists_modification(bs_t* b, h265_slice_header_t* hrd)
{
    ...
    if (hrd->ref_pic_lists_modification.ref_pic_list_modification_flag_l0)
    {
        ...
        if (numRpsCurrTempList0 > 1)
        {
            ...
            for (int i = 0; i <= hrd->num_ref_idx_l0_active_minus1; i++) // ×¢ÒâÓÐµÈºÅ£¬Òª×¢Òâ±ß½ç
            {
    ...
    if (hrd->slice_type == H265_SH_SLICE_TYPE_B)
    {
        ...
        if (hrd->ref_pic_lists_modification.ref_pic_list_modification_flag_l1)
        {
            ...
            if (numRpsCurrTempList1 > 1)
            {
                ...
                for (int i = 0; i <= hrd->num_ref_idx_l1_active_minus1; i++) // ×¢ÒâÓÐµÈºÅ£¬Òª×¢Òâ±ß½ç
                {
...
{
    // NOTE ²»ÄÜÖ±½Ó¸³Öµ¸øsps£¬ÒòÎª»¹Î´ÖªÊÇÄÄÒ»¸ösps¡£
    ...
    if (sps->conformance_window_flag)
    {
        ...
        // ¸ù¾ÝTable6-1¼°7.4.3.2.1ÖØÐÂ¼ÆËã¿í¡¢¸ß
        // ×¢Òâ£ºÊÖ²áÀï¼Ó1£¬Êµ¼ÊÉÏ²»ÓÃ
    ...
    sps->m_RPSList.resize(sps->num_short_term_ref_pic_sets); // È·¶¨Ò»¹²ÓÐ¶àÉÙ¸öRPSÁÐ±í
...
void h265_read_slice_header(h265_stream_t* h, bs_t* b)
{
    ...
    if (!hrd->dependent_slice_segment_flag)
    {
        ...
        else
        {
            ...
            if (!hrd->short_term_ref_pic_set_sps_flag)
            {
                ...
                // sps->num_short_term_ref_pic_setÊµ¼ÊµÈÓÚssps->m_RPSList.size() ÏÂÍ¬
    ...
    if (pps->tiles_enabled_flag || pps->entropy_coding_sync_enabled_flag)
    {
        ...
        if (hrd->num_entry_point_offsets > 0)
        {
            ...
            {
                // to confirm
                // entry_point_offset_minus1Îªu(u),³¤¶ÈÔÚoffset_len_minus1µÄÖµ¼ÓÉÏ1£¬¼ûÉÏ
