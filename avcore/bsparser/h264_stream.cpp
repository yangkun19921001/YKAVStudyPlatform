...
void read_slice_header(h264_stream_t *h, bs_t *b) {
    ...
    if (is_slice_type(sh->slice_type, SH_SLICE_TYPE_P) || is_slice_type(sh->slice_type, SH_SLICE_TYPE_SP) ||
...
void debug_sps(sps_t *sps) {
    ...
    for (SchedSelIdx = 0; SchedSelIdx <= sps->hrd.cpb_cnt_minus1; SchedSelIdx++) {
        printf("   bit_rate_value_minus1[%d] : %d \n", SchedSelIdx,
               sps->hrd.bit_rate_value_minus1[SchedSelIdx]); // up to cpb_cnt_minus1, which is <= 31
