...
static void do_video_out(OutputFile *of,
    ...
    if (!next_picture) {
        ...
        printf("sync_ipts=%f sync_opts=%lld delta=%f drop_frame=%d duration=%f\n", sync_ipts, ost->sync_opts, delta,
               delta < -1.1, duration);
        ...
        if (format_video_sync == VSYNC_AUTO) {
            if (!strcmp(of->ctx->oformat->name, "avi")) {
                ...
                format_video_sync = (of->ctx->oformat->flags & AVFMT_VARIABLE_FPS) ? ((of->ctx->oformat->flags &
                                                                                       AVFMT_NOTIMESTAMPS)
    ...
    for (i = 0; i < nb_frames; i++) {
        ...
        if (ost->forced_kf_index < ost->forced_kf_count &&
            ...
            printf("force_key_frame: n:%f n_forced:%f prev_forced_n:%f t:%f prev_forced_t:%f -> res:%f\n",
        ...
        while (1) {
            ...
            switch (av_buffersink_get_type(filter)) {
                case AVMEDIA_TYPE_VIDEO:
                    ...
                    printf("filtered_frame=%lld  float_pts=%f\n", filtered_frame->pts, float_pts);

                    if (filtered_frame->pts == 227) {
                        printf("filtered_frame test\n");
                    }
...
static int ifilter_send_frame(InputFilter *ifilter, AVFrame *frame) {
    ...
    if (!frame) {
        printf("传入结束过滤器包---------------av_buffersrc_add_frame_flags--------------");
    }
...
static int decode(AVCodecContext *avctx, AVFrame *frame, int *got_frame, AVPacket *pkt) {
    ...
    *got_frame = 0;
    ...
    if (pkt && pkt->pts == 110800)
        printf("decode pck pts %lld \n", pkt->pts);
    if (pkt) {
...
static int decode_audio(InputStream *ist, AVPacket *pkt, int *got_output,
    ...
    if (decoded_frame->pts != AV_NOPTS_VALUE)
        decoded_frame->pts = av_rescale_delta(decoded_frame_tb, decoded_frame->pts,
                                              (AVRational) {1, avctx->sample_rate}, decoded_frame->nb_samples,
                                              &ist->filter_in_rescale_delta_last,
    ...
    if (ist->st->codecpar->video_delay < ist->dec_ctx->has_b_frames) {
        if (ist->dec_ctx->codec_id == AV_CODEC_ID_H264) {
            ...
            av_log(ist->dec_ctx, AV_LOG_WARNING,
                   "video_delay is larger in decoder than remuxer %d > %d.\n"
        ...
        case AVMEDIA_TYPE_VIDEO:
            ...
            if (av_q2d(enc_ctx->time_base) < 0.001 && video_sync_method != VSYNC_PASSTHROUGH
                && (video_sync_method == VSYNC_CFR || video_sync_method == VSYNC_VSCFR ||
                    (video_sync_method == VSYNC_AUTO && !(oc->oformat->flags & AVFMT_VARIABLE_FPS)))) {
...
//        av_opt_set(&dictionary, "profile", "main", AV_OPT_SEARCH_CHILDREN);
//        av_opt_set(&dictionary, "threads", "1", 0);
//        if ((ret = avcodec_open2(ost->enc_ctx, codec, &dictionary)) < 0) {
            if (ret == AVERROR_EXPERIMENTAL)
...
static int process_input(int file_index) {
    ...
    if (debug_ts) {
        av_log(NULL, AV_LOG_INFO, "remuxer -> ist_index:%d type:%s "
    ...
    if (debug_ts) {
        av_log(NULL, AV_LOG_INFO,
               "remuxer+ffmpeg -> ist_index:%d type:%s pkt_pts:%s pkt_pts_time:%s pkt_dts:%s pkt_dts_time:%s off:%s off_time:%s\n",
               ifile->ist_index + pkt.stream_index, av_get_media_type_string(ist->dec_ctx->codec_type),
               av_ts2str(pkt.pts), av_ts2timestr(pkt.pts, &ist->st->time_base),
               av_ts2str(pkt.dts), av_ts2timestr(pkt.dts, &ist->st->time_base),
               av_ts2str(input_files[ist->file_index]->ts_offset),
