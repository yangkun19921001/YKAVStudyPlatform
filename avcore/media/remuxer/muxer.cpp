...
void Muxer::doReMuxer(string srcPath,string dPath)
{
    ...
    if ((ret = avformat_open_input(&in_fmtCtx,srcPath.c_str(),NULL,NULL)) < 0) {
        printf("avformat_open_input fail %d",ret);
        return;
    }
    ...
    while (av_read_frame(in_fmtCtx, sPacket) >= 0) {
        ...
        if (sPacket->stream_index == in_audio_index) {
            ...
            static int sum = 0;
            sum++;
            printf("video sum(%d) size(%d) pts %d(%s) dts%d(%s) index %d",sum,sPacket->size,sPacket->pts,av_ts2timestr(sPacket->pts,&ou_video_stream->time_base),sPacket->dts,av_ts2timestr(sPacket->dts,&ou_video_stream->time_base),sPacket->stream_index);
            if (av_write_frame(ou_fmtCtx, sPacket) < 0) {
                printf("av_write_frame 2 fial");
                break;
            }
        }
...
void Muxer::doReMuxerWithStream(string srcPath,string dPath)
{
    ...
    while (true) {
        ...
        if (sPacket->stream_index == in_video_index) {  // 暂时只考虑视频
            static int sum = 0;
            sum++;
            printf("video sum(%d) size(%d) pts %d dts %d index %d",sum,sPacket->size,sPacket->pts,sPacket->dts,sPacket->stream_index);
...
void* Muxer::consum_thread(void *opaque)
{
    ...
    while (av_read_frame(in_fmtCtx, sPacket) >= 0) {
        ...
        static int sum = 0;
        sum++;
        printf("video sum(%d) size(%ld) pts %d(%s) dts%d(%s) index %d",sum,sPacket->size,sPacket->pts,av_ts2timestr(sPacket->pts,&ou_video_stream->time_base),sPacket->dts,av_ts2timestr(sPacket->dts,&ou_video_stream->time_base),sPacket->stream_index);
...
int Muxer::readVideoPacket(void *client,uint8_t* buf,int buflen)
{
    ...
    if (pkt != NULL) {
        static int i = 0;
        i++;
        printf("consum video pkt %ld size %d",i,pkt->size);
        if (pkt->size > buflen) {
...
void Muxer::doMuxerTwoFile(string aduio_srcPath,string video_srcPath,string dpath)
{
    ...
    if (ret < 0) {
        printf("avformat_open_input audio fail %d",ret);
        return;
    }
    ret = avformat_find_stream_info(audio_fmtCtx,NULL);
    if (ret < 0) {
        printf("avformat_find_stream_info audio fail");
    ...
    if (ret < 0) {
        printf("avformat_open_input video fail %d",ret);
    ...
    if (ret < 0) {
        printf("avformat_alloc_output_context2 fail %d",ret);
        releaseResource(&audio_fmtCtx, &video_fmtCtx, NULL);
        return;
    }
    ...
    if (video_stream_index != -1) {
        ...
        if (avcodec_parameters_copy(video_ou_stream->codecpar,video_in_stream->codecpar) < 0) {
            printf("avcodec_parameters_copy null");
    ...
    if ((ret = avformat_write_header(out_fmtCtx,NULL)) < 0){
        printf("avformat_write_header %d",ret);
        releaseResource(&audio_fmtCtx, &video_fmtCtx, &out_fmtCtx);
        return;
    }
    ...
    do {
        if (!audio_finish && !found_audio && audio_stream_index != -1) {
            ...
            if (!audio_finish) {
                ...
                printf("audio pts:%s dts:%s duration %s size %d finish %d",
        ...
        if (!video_finish && !found_video && video_stream_index != -1) {
            ...
            if (!video_finish) {
                ...
                static int sum=0;
                sum++;
                printf("video pts:%s dts:%s duration %s size %d key:%d finish %d sum %d",
...
//            AVRational tb = video_in_stream->time_base;
//            printf("video pts:%s dts:%s duration %s size %d key:%d finish %d",
//                 av_ts2timestr(vSPacket->pts,&tb),
//                 av_ts2timestr(vSPacket->dts,&tb),
//                 av_ts2timestr(vSPacket->duration,&tb),
        ...
        if (found_video && found_audio && !video_finish && !audio_finish) {
            if ((ret = av_compare_ts(aSPacket->pts,audio_ou_stream->time_base,
                ...
                if ((ret = av_write_frame(out_fmtCtx,vSPacket)) < 0) {
                    printf("av_write_frame video 1 fail %d",ret);
                    releaseResource(&audio_fmtCtx, &video_fmtCtx, &out_fmtCtx);
                    break;
                }
                printf("写入了视频 1");
                ...
                if ((ret = av_write_frame(out_fmtCtx,aSPacket)) < 0) {
                    printf("av_write_frame audio 1 fail %d",ret);
                    releaseResource(&audio_fmtCtx, &video_fmtCtx, &out_fmtCtx);
                    break;
                }
                printf("写入了音频 1");
            ...
            // 写入视频
            if ((ret = av_write_frame(out_fmtCtx,vSPacket)) < 0) {
                printf("av_write_frame video 2 fail %d",ret);
                releaseResource(&audio_fmtCtx, &video_fmtCtx, &out_fmtCtx);
                break;
            }
            printf("写入了视频 2");
            ...
            // 写入音频
            if ((ret = av_write_frame(out_fmtCtx,aSPacket)) < 0) {
                printf("av_write_frame audio 2 fail %d",ret);
                break;
            }
            printf("写入了音频 2");
            found_audio = false;
...
int main(){
    ...
    const char * aUrl = "https://clipres.yishihui.com/longvideo/material/voice/bgm/860e2264d55f4f0cbb9ba62cf856649b.mp3";
    const char * vUrl = "/Users/devyk/Downloads/speed_fileaTkx5IUBs3dNJHbrln7i8Xevy401jWom.mp4";
    const char * oUrl = "/Users/devyk/Data/Project/sample/github_code/YKAVStudyPlatform/temp/out.mp4";

    test->doMuxerTwoFile(aUrl,vUrl,oUrl);
    return 1;
}
